...
// 		@"enableTweak": @NO,
// 		@"colorPrefCustom":	@"Default",
//
// 		@"colorPrefCustom":	[NSNumber numberWithInteger:0],
// 		@"shortHoldTime":	[NSNumber numberWithFloat:0.2],
// 	}];
//
// 	enableTweak = [preferences boolForKey:@"enableTweak"];
//
// 	colorPrefCustom = [preferences objectForKey:@"colorPrefCustom"];
//
//
//
//
      ...
      %orig(arg1);
		...
		//else the tweak is enabled <start>
			...
			if (posPrefChoice < 0 && posPrefChoice >= 6) {
				...
				// if ((posPrefChoice < 0 && posPrefChoice >= 6) && [sizePrefChoice isEqualToString:@"Custom"]) {
...
%ctor {
	@autoreleasepool{
		...
		loadPreferences();
		//CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(), NULL, CFNotificationCallback(loadPreferences), CFSTR(PreferencesChangedNotification), NULL, CFNotificationSuspensionBehaviorCoalesce);
	}
}
